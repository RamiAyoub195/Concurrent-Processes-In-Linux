...
void ta_process(int ta_number, int studentList[]){
    ...
    while (counter < 3){
        ...
        if (sem_trywait(semaphores[(ta_number + 1) % 5]) == -1){ //Try locking semaphore 1
            sem_post(semaphores[ta_number]); //If semaphore 1 is already locked, unlock semaphore 1
            continue;
        }
        ...
        usleep(100000);  //0.1 second delay to prevent deadlocks
